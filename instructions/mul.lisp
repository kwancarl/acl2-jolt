...
(define mul-semantics-32 ((x (unsigned-byte-p 32 x)) (y (unsigned-byte-p 32 y)))
  (b* (((unless (unsigned-byte-p 32 x)) 0)
       ((unless (unsigned-byte-p 32 y)) 0)
       ;; MUL in circuit
       (z (* (logext 32 x) (logext 32 y)))
       ;; CHUNK
...
(define mul-32 ((x (unsigned-byte-p 32 x)) (y (unsigned-byte-p 32 y)))
  :verify-guards nil
  :enabled t
  (b* (((unless (unsigned-byte-p 32 x)) 0)
       ((unless (unsigned-byte-p 32 y)) 0)
...
(defthm mul-32-mul-semantics-32-equiv
 (equal (mul-32 x y) (mul-semantics-32 x y))
 :hints (("Goal" :in-theory (e/d (mul-semantics-32)
                                 ((:e materialize-identity-subtable)
                                  (:e materialize-truncate-subtable))))))


;; SEMANTIC CORRECTNESS OF MULU
(fgl::def-fgl-thm mul-semantics-32-correctness
 :hyp (and (unsigned-byte-p 32 x) (unsigned-byte-p 32 y))
 :concl (equal (mul-semantics-32 x y) (loghead 32 (* (logext 32 x) (logext 32 y)))))

(defthm mul-32-correctness
 (implies (and (unsigned-byte-p 32 x) (unsigned-byte-p 32 y))
          (equal (mul-32 x y) (loghead 32 (* (logext 32 x) (logext 32 y)))))) 



;; 64-BIT VERSION

(define mul-semantics-64 ((x (unsigned-byte-p 64 x)) (y (unsigned-byte-p 64 y)))
  (b* (((unless (unsigned-byte-p 64 x)) 0)
       ((unless (unsigned-byte-p 64 y)) 0)
       ;; mul in circuit
       (z (* (logext 64 x) (logext 64 y)))
       ;; CHUNK
...
(define mul-64 ((x (unsigned-byte-p 64 x)) (y (unsigned-byte-p 64 y)))
  :verify-guards nil
  :enabled t
  (b* (((unless (unsigned-byte-p 64 x)) 0)
       ((unless (unsigned-byte-p 64 y)) 0)
...
(defthm mul-64-mul-semantics-64-equiv
 (equal (mul-64 x y) (mul-semantics-64 x y))
 :hints (("Goal" :in-theory (e/d (mul-semantics-64)
                                 ((:e materialize-identity-subtable)
                                  (:e materialize-truncate-subtable))))))

;; SEMANTIC CORRECTNESS OF MULU
(fgl::def-fgl-thm mul-semantics-64-correctness
 :hyp (and (unsigned-byte-p 64 x) (unsigned-byte-p 64 y))
 :concl (equal (mul-semantics-64 x y) (loghead 64 (* (logext 64 x) (logext 64 y)))))

(defthm mul-64-correctness
 (implies (and (unsigned-byte-p 64 x) (unsigned-byte-p 64 y))
          (equal (mul-64 x y) (loghead 64 (* (logext 64 x) (logext 64 y))))))
