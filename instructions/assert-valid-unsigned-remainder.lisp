...
(define assert-valid-unsigned-remainder-semantics-32
  ...
  (b* (;; Edge cases
       ...
       (r3   (if (= y8-3 0) 1 0)))
      ;; Combine
      (+ z0 (* z1 w0) (* z2 w0 w1) (* z3 w0 w1 w2) (* r0 r1 r2 r3))))

;; Correctness of AssertValidUnsignedRemainder intermediate semantics layer
(gl::def-gl-thm assert-valid-unsigned-remainder-semantics-32-correctness
 :hyp (and (unsigned-byte-p 32 x) (unsigned-byte-p 32 y))
 :concl (equal (assert-valid-unsigned-remainder-semantics-32 x y)
	       (if (or (= y 0) (< x y)) 1 0))
 :g-bindings (gl::auto-bindings (:mix (:nat x 32) (:nat y 32))))


;; Define AssertValidUnsignedRemainder with subtable lookups
(define assert-valid-unsigned-remainder-32
    ((x (unsigned-byte-p 32 x)) (y (unsigned-byte-p 32 y)))
    :verify-guards nil
  (b* (;; Edge cases
       ((unless (unsigned-byte-p 32 x)) 0)
...
;; Equivalence between assert-valid-unsigned-remainder-32 & its intermediate semantics version
(defthm assert-valid-unsigned-remainder-32-assert-valid-unsigned-remainder-semantics-32-equiv
  (equal (assert-valid-unsigned-remainder-32 x y)
         (assert-valid-unsigned-remainder-semantics-32 x y))
 :hints (("Goal" :in-theory (enable assert-valid-unsigned-remainder-32
                                    assert-valid-unsigned-remainder-semantics-32))))

;; Correctness of AssertValidUnsignedRemainder
(defthm assert-valid-unsigned-remainder-32-correctness
 (implies (and (unsigned-byte-p 32 x) (unsigned-byte-p 32 y))
          (equal (assert-valid-unsigned-remainder-32 x y) (if (or (= y 0) (< x y)) 1 0))))


;; 64-BIT VERSION

(define assert-valid-unsigned-remainder-semantics-64
...
(define assert-valid-unsigned-remainder-64
    ((x (unsigned-byte-p 64 x)) (y (unsigned-byte-p 64 y)))
  :verify-guards nil
  (b* (((unless (unsigned-byte-p 64 x)) 0)
       ((unless (unsigned-byte-p 64 y)) 0)
...
(defthm assert-valid-unsigned-remainder-64-assert-valid-unsigned-remainder-semantics-64-equiv
 (equal (assert-valid-unsigned-remainder-64 x y)
        (assert-valid-unsigned-remainder-semantics-64 x y))
 :hints (("goal" :in-theory (enable assert-valid-unsigned-remainder-64
                                    assert-valid-unsigned-remainder-semantics-64))))

;; Semantic correctness of assert-valid-unsigned-remainder
(gl::def-gl-thm assert-valid-unsigned-remainder-semantics-64-correctness
 :hyp (and (unsigned-byte-p 64 x) (unsigned-byte-p 64 y))
 :concl (equal (assert-valid-unsigned-remainder-semantics-64 x y)
	       (if (or (= y 0) (< x y)) 1 0))
 :g-bindings (gl::auto-bindings (:mix (:nat x 64) (:nat y 64))))

;; Correctness of assert-valid-unsigned-remainder
(defthm assert-valid-unsigned-remainder-64-correctness
 (implies (and (unsigned-byte-p 64 x) (unsigned-byte-p 64 y))
          (equal (assert-valid-unsigned-remainder-64 x y) (if (or (= y 0) (< x y)) 1 0))))
