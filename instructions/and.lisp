...
(define and-32 ((x (unsigned-byte-p 32 x)) (y (unsigned-byte-p 32 y)))
  ...
  (b* (((unless (unsigned-byte-p 32 x)) 0)
       ...
       ;; LOOKUPS
       (w0   (lookup x8-0 y8-0 and-subtable))
       (w1   (lookup x8-1 y8-1 and-subtable))
       (w2   (lookup x8-2 y8-2 and-subtable))
       (w3   (lookup x8-3 y8-3 and-subtable)))
      ;; COMBINE
      (merge-4-u8s w0 w1 w2 w3)))
...
(define and-64 ((x (unsigned-byte-p 64 x)) (y (unsigned-byte-p 64 y)))
  ...
  (b* (((unless (unsigned-byte-p 64 x)) 0)
       ...
       (w2   (lookup x8-2 y8-2 and-subtable))
       (w3   (lookup x8-3 y8-3 and-subtable))
       (w4   (lookup x8-4 y8-4 and-subtable))
       (w5   (lookup x8-5 y8-5 and-subtable))
       (w6   (lookup x8-6 y8-6 and-subtable))
       (w7   (lookup x8-7 y8-7 and-subtable)))
      ;; COMBINE
      (merge-8-u8s w0 w1 w2 w3 w4 w5 w6 w7)))
