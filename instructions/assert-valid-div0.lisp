...
(define assert-valid-div0-32 ((x (unsigned-byte-p 32 x)) (y (unsigned-byte-p 32 y)))
 ...
 (local
  (gl::def-gl-thm auxiliary-lemma-1
   :hyp (and (unsigned-byte-p 32 y)
              (equal (loghead 8 y) (+ -1 (expt 2 8)))
              (equal (logtail 24 y) (+ -1 (expt 2 8)))
              (equal (loghead 8 (logtail 8 y)) (+ -1 (expt 2 8)))
              (equal (loghead 8 (logtail 16 y)) (+ -1 (expt 2 8))))
