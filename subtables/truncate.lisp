...
;; Returns a list of pairs (cons i mask) for all i in [0, x-hi]
...
;; Given a list where each element `idx` is a pair `(cons x mask)`, generated by `truncate-indices`,
;; returns a list of pairs `(cons idx (logand x mask))`
;; (defun materialize-truncate-subtable (x-hi mask)
...
(defun materialize-truncate-subtable (x-hi mask)
 (b* (or (not (natp x-hi)) (not (natp mask)))
     nil
     (if (zerop x-hi)
...
;;  (alistp (materialize-truncate-subtable x-hi mask)))
 ...
 (implies (and (natp x-hi) 
          ...
          (b* ((subtable (materialize-truncate-subtable x-hi mask)))
              (equal (single-lookup i subtable)
                     (logand i mask))))
 :hints (("Goal" :in-theory (e/d (lookup) (materialize-truncate-subtable))
